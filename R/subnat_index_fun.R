...
#' @examples
#' subnat_FPsource_data <- mcmsupply::subnat_FPsource_data
#' country_subnat_tbl <- subnat_FPsource_data %>%
#' dplyr::group_by(Country, Region) %>%
#' dplyr::select(Country, Region) %>%
#' dplyr::distinct() # table of country and regions (removes repeats in region names)
#' subnat_index_fun(subnat_FPsource_data, country_subnat_tbl$Region, country_subnat_tbl$Country)
#' @export
